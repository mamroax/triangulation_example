...
from triangulation_example.functions import draw_line

try:
    x = []
    y = []
    points_file = open('points.txt', 'r')  # откроем файл с координатами
    triangles_file = open('triangles.txt', 'r')  # откроем файл с треугольниками
    ...
    for j in range(len(triangles_list)):
        draw_line(triangles_list[j], points_line)
